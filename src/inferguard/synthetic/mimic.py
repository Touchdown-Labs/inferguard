...
def make_job(
    ...
    env = {
        ...
        "GMI_MODEL_PATH": env_or_default(
            cluster.get("model_path_env"), model_profile.get("hf_repo", "MODEL_PATH_FROM_ENV")
        ),
        "GMI_MODEL_CONFIG_PATH": env_or_default(cluster.get("model_config_path_env"), ""),
        "GMI_CONTAINER_IMAGE": env_or_default(
            cluster.get("container_image_env"), "CONTAINER_IMAGE_FROM_ENV"
        ),
        "GMI_ENDPOINT": env_or_default(
...
def env_or_default(env_name: str | None, default: str) -> str:
    if not env_name:
        return default
    return os.environ.get(env_name, f"${{{env_name}}}" if default else "")
