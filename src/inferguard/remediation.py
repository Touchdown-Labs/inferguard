...
class RemediationResult:
    ...
    estimated_capacity_impact_pct: float | None = None
    estimated_kv_relief_pct: float | None = None
    cost_note: str = ""
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "estimated_capacity_impact_pct": self.estimated_capacity_impact_pct,
            "estimated_kv_relief_pct": self.estimated_kv_relief_pct,
...
def generate_fix(
    ...
    estimated_capacity_impact_pct: float | None = None
    estimated_kv_relief_pct: float | None = None
    cost_note = "Impact estimate unavailable for this remediation."
    ...
    if failure_mode == "kv_saturation":
        ...
        estimated_capacity_impact_pct = -10.0
        estimated_kv_relief_pct = 15.0
        cost_note = "FP8 KV reduces memory pressure with modest throughput risk and some quality-validation overhead."
        if family == "deepseek-r1":
        ...
        estimated_capacity_impact_pct = 20.0
        cost_note = "Queue-drain tuning may improve throughput if memory headroom exists, but can worsen contention if the diagnosis is wrong."
        ...
        estimated_capacity_impact_pct = -50.0
        estimated_kv_relief_pct = 20.0
        cost_note = "Concurrency throttling trades peak throughput for stability and lower waste under preemption storms."
        if family == "deepseek-r1":
        ...
        estimated_capacity_impact_pct = -15.0
        estimated_kv_relief_pct = 10.0
        cost_note = "Reducing swap can recover latency tail but may require lower effective concurrency."
        ...
        estimated_capacity_impact_pct = 5.0
        cost_note = "TTFT-focused tuning can improve responsiveness and effective utilization if prompt reuse is real."
    ...
    return RemediationResult(
        ...
        estimated_capacity_impact_pct=estimated_capacity_impact_pct,
        estimated_kv_relief_pct=estimated_kv_relief_pct,
        cost_note=cost_note,
    )
