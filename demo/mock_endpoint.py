...
APPLIED_ACTIONS_KEY = web.AppKey("applied_actions", list)
...
async def handle_actuate(request: web.Request) -> web.Response:
    ...
    request.app[APPLIED_ACTIONS_KEY].append(body)
    if action_type == "throttle_concurrency":
        return web.json_response({"accepted": True})
    return web.json_response({"accepted": False, "error": f"unsupported action: {action_type}"})
...
def create_app(engine: str, model_id: str, scenario: str = "healthy") -> web.Application:
    ...
    app[APPLIED_ACTIONS_KEY] = []
    app.router.add_get("/metrics", handle_metrics)
    app.router.add_get("/v1/models", handle_models)
    app.router.add_get("/health", handle_health)
    app.router.add_post("/inferguard/actuate", handle_actuate)
