...
- **L3** (`rlm_agent/`) — Standardized production-ready AI brain with RLM decomposer + Daytona canary client. No `inferguard.*` imports.
...
- `rlm_agent/*.py` must NEVER import from `inferguard.*`. L3 uses dict payloads, not shared types.
...
| `rlm_agent/brain.py` | InferGuardBrain — W1-W4 decomposition prompt, GMI call, advisory emission |
| `rlm_agent/rlm_decomposer.py` | `rlm` package wrapper with direct-GMI fallback |
| `rlm_agent/daytona_client.py` | Daytona canary client with graceful degradation |
| `rlm_agent/app.py` | FastAPI agent service entrypoint |
